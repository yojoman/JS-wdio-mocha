...
// Вопросы:
// 1) Не получается задать значение телу письма, текст либо не задается, либо улетает в "тему" письма, перепробовал различные локаторы и функции browser.elementClick, browser.keys, elementSendKeys, SendKeys, setValue. (см. функцию fillLetter() в secure_page.js)
// 2) Как корректно останавливать тест во время выполнения?
// 3) В общем хотелось бы услышать что не так, что можно было сделать лучше, любые советы приветствуются!
